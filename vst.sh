...
help() {
  ...
  -l, --lang              Default language                default: ru
  ...
  Example: bash $0 -e admin@lunexy.com -p p4ssw0rd --apache no --phpfpm yes"
    exit 1
}
...
if [ "$iptables" = 'yes' ] && [ "$fail2ban" = 'no' ]; then
    echo -n ' + Fail2Ban'
fi
echo -e "\n\n"
...
echo "deb http://nginx.org/packages/mainline/debian/ $codename nginx" > $apt/nginx.list
...
echo -e "Congratulations, server just successfully installed \
❐ Lunexy Control Space
...
--
Sincerely yours
lunexy.com - Unlimited Digital Space
" > $tmpfile

send_mail="$VESTA/web/inc/mail-wrapper.php"
cat $tmpfile | $send_mail -s "❐ Lunexy Control Space" $email
